# FinApiResTfulServices.AuthorizationApi
...
<a name="getToken"></a>
# **getToken**
> AccessToken getToken(grantType, clientId, clientSecret, opts)
...
var apiInstance = new FinApiResTfulServices.AuthorizationApi();
...
var opts = { 
  'refreshToken': "refreshToken_example", // String | Refresh token. Required for grant_type=refresh_token only.
...
apiInstance.getToken(grantType, clientId, clientSecret, opts, callback);
```
...
var opts = { 
  'tokenTypeHint': "tokenTypeHint_example" // String | A hint about the type of the token submitted for revocation
};
...
apiInstance.revokeToken(token, opts, callback);
```
